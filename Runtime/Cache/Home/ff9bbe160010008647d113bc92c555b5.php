...
<!-- 导航结束 -->
<link rel="stylesheet" type="text/css" href="<?php echo (CSS_HOME_URL); ?>price.css">
<!-- 用户报价开始 -->
	<div id="price">
		<div class="main">
			<div class="price_top">
				<img src="<?php echo (IMG_HOME_URL); ?>price.png" height="65"><span>域名详情</span><img src="<?php echo (IMG_HOME_URL); ?>jiao.png" height="65">
			</div>
	    <form action="/index.php/Home/User/price/yuming_id/187/cat_id/33" method="post">
			<div class="price_content clearfix">
			    <h2><?php echo ($priceid[0][yuming_name]); ?></h2>
			    <div class="price_content1 fl">
				    <div style="width: 460px;height: 190px;background-color: #f6f6f6; color: #666"><?php echo ($priceid[0][yuming_introduce]); ?></div>
				    <p>如果您感兴趣，请在下面提交您的报价，卖家就会对您的报价作出相应回复。</p>
			    </div>
				<div class="price_content2 fl">
					<p>
						<span class="price_title">标价：</span>
						<span style="color: #ff5d15;">￥<?php echo ($priceid[0][yuming_xiaoshou_price]); ?></span>
					</p>
					<p class="second">
						<span class="price_title">您的报价：</span>
						<input id="focuss" class="txt" type="text" id="numberbox" placeholder="您的报价" name="user_baojia"/><i></i>
					</p>
					<p>
			...
			<div class="price_foot">
				<p>您可能感兴趣的域名：</p>
				<ul>
				 <?php if(is_array($ls_ids)): foreach($ls_ids as $key=>$v): ?><li><a href="<?php echo u('User/price',array('yuming_id'=>$v['yuming_id'],'cat_id'=>$v['cat_id']));?>"><?php echo ($v["yuming_name"]); ?></a></li><?php endforeach; endif; ?>
				</ul>
			</div>
		</div>	
	</div>
